...
class Intermedia extends CI_Model
{
    private $nombreTabla;
    private $idTabla;
    ...
    {
        parent::__construct();
        $this->nombreTabla = 'intermedia';
        $this->idTabla = 'idintermedia';
    }
    /* CRUD */
    // CREATE
    ...
    public function delete($id) // Eliminar registro
    {
     ...
     public function obtenerPorPelicula($id)
     {
         $this->db->select('intermedia.*, usuario.nombre');
         $this->db->where('intermedia.idpelicula', $id);
         $this->db->join('usuario', 'intermedia.idusuario = usuario.idusuario');
